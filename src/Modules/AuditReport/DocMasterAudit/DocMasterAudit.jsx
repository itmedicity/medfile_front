...
import { getdocMasterEditAuditReports, getDocumentCreateAuditReports } from '../../../api/commonAPI';
import DefaultPageLayout from '../../../Components/DefaultPageLayout';
import { AuditformatDate, AuditMastcolumnsByTab, searchIconStyle, searchInputStyle } from '../AuditCommonCodes/auditCommonStyle';
import DateRangeFilter from '../AuditCommonCodes/DateRangeFilter';

const DocMasterAudit = () => {
    ...
    const { isLoading: isLoadingCreated, data: createdData } = useQuery({
        ...
        queryFn: getDocumentCreateAuditReports,
        // staleTime: Infinity,
    ...
    const { isLoading: isLoadingEdited, data: editedData } = useQuery({
        ...
        queryFn: getdocMasterEditAuditReports,
        // staleTime: Infinity,
        refetchOnWindowFocus: false,
    });
    ...
    const dateFilteredLogs = auditLogs.filter((log) => {
        try {
            ...
            const logDate = new Date(log.timestamp || log.audit_log_time || log.created_time_date);
                    ...
                    <Table>
                        <TableHead>
                            <TableRow sx={{ bgcolor: '#f4f6f8' }}>
                                {AuditMastcolumnsByTab[selectedTab].map((col) => (
                        ...
                        <TableBody>
                            {(isLoadingCreated || isLoadingEdited) ? (
                                <TableRow>
                                    <TableCell
                                        colSpan={AuditMastcolumnsByTab[selectedTab].length}
                                        align="center"
                                ...
                                filteredLogs.map((log, index) => (
                                    ...
                                    >
                                        {AuditMastcolumnsByTab[selectedTab].map((col) => {
                                            ...
                                            if (col.key === 'created_time_date' || col.key === 'audit_log_time') {
...
export default memo(DocMasterAudit);
