...
const FileUpload = () => {
    ...
    const handleDocInformationSubmit = useCallback(async (e) => {
        ...
        if (Number(documentState.institute) === 2 && Number(documentState.institute) === 0) {
            warningNofity('Institute cannot be empty')
            return
        }

        if (Number(documentState.institute) === 2 && Number(documentState.course) === 0) {
        ...
        if (Boolean(documentState.isRequiredExp) === true && isValid(new Date(documentState.docExpStart)) === false) {
            warningNofity('Document Expiry Start Date cannot be empty || Valid Date is required')
            return
        }

        if (Boolean(documentState.isRequiredExp) === true && isValid(new Date(documentState.docExpEnd)) === false) {
            warningNofity('Document Expiry End Date cannot be empty || Valid Date is required')
            return
        }

        if (Boolean(documentState.isRequiredExp) === true && (new Date(documentState.docExpStart) > new Date(documentState.docExpEnd))) {
            warningNofity('Document Expiry Start Date cannot be greater than Expiry End Date')
            return
        }
        ...
        try {
            ...
            if (success === 0) {
                ...
                queryClient.invalidateQueries('getDocumentNumber')
                setDocumentState({
                    docNumber: 0,
                    docName: "",
                    docDes: "",
        ...
        >
            ...
            <TabContext value={value}>
                <Box sx={{ borderBottom: 1.5, borderColor: "divider" }} >
                    <TabList onChange={handleChange} aria-label="lab API tabs example" sx={{ minHeight: 0 }} className="flex justify-end items-center" >
                        <Tab
                ...
                <TabPanel value="1" className="overflow-scroll" sx={{ p: 1 }} >
                    <Box className="">
                                ...
                                >
                                    <Grid size={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }}>
                                        <CustomInput
                                            ...
                                            value={docName}
                                            onChange={(e) => handleDocumentState({ target: { name: "docName", value: e.target.value } })}
                                        />
                                    </Grid>
                                    <Grid size={{ xs: 12, sm: 12, md: 12, lg: 12, xl: 12 }}>
                                        <Textarea
                                            ...
                                            minRows={1}
                                            value={docDes}
                                            onChange={(e) => handleDocumentState({ target: { name: "docDes", value: e.target.value } })}
                                            sx={{
                                                fontSize: 15,
                                                "&.MuiTextarea-root": {
                        ...
                        >
                            ...
                            <Box className="flex flex-1 bg-red-300">s</Box>
