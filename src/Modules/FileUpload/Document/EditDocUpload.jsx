...
const EditDocUpload = ({ params }) => {
  const { doc_slno } = params.row; // DATA FROM TABLE ACTION || FROM THE PARAMS

  const docmntID = doc_slno; // ID FOR GETTIG THE DOC DETAILS
  ...
  const { isLoading, data, error } = useQuery({
    queryKey: ["getDocInfoByID", docmntID],
    queryFn: () => getDocInforByID(docmntID),
    enabled: !!docmntID,
  });
                ...
                <Box className="p-3">
                  ...
                  <Box className="flex justify-between">
                    ...
                    <Typography
                      ...
                      sx={{ display: isRequiredExp === true ? "flex" : "none" }}
                      level="body-sm"
